...
namespace TreePrompt2Json.PromptBuilder.MVVM.ViewModels
{
    partial class PromptEditorVM : ObservableObject, IContentVM
    {
        ...
        private IThemeColorService _themeColorService;
        ...
        private static string[] IconList = [TextIcon, JsonIcon];
    ...
    partial class PromptEditorVM
    {
            ...
            if (para is DragEventArgs e)
            {
                if (e.Data.GetDataPresent(DataFormats.FileDrop))
                {
                    ...
                    try
                    {
                        ...
                        this.PromptPacketList.Add(CreatePromptPacket(IconList, entranceName, root));
                    }
                    catch (Exception ex)
                    {
                        // 反序列化失败，视为 string
                            ...
                            using (var reader = new BinaryReader(stream))
                            {
                                ...
                                if (chunk != null)
                                {
                                    ...
                                    if (IsBase64String(content))
                                    {
                                        ...
                                        this.PromptPacketList.Add(CreatePromptPacket(IconList, "text", CreatePromptString(decodedText)));
                                    }
                                    else
                                    {
                                        this.PromptPacketList.Add(CreatePromptPacket(IconList, "text", CreatePromptString($"{content}")));
                                    }
        ...
        private void OnAddJsonPrompt()
        {
            ...
            this.PromptPacketList.Add(CreatePromptPacket(IconList, "json", new ToggleTreeViewNode()
            {
                Enable = true,
                Text = string.Empty,
                UseDelayRender = true,
                ContentRenderType = ContentRenderType.ForJsonEditor
            }.WithChild(new ToggleTreeViewNode()
            {
                Enable = true,
                Text = "Root"
            })));
        ...
        private void OnAddTxtPrompt()
        {
            ...
            this.PromptPacketList.Add(CreatePromptPacket(IconList, "text", CreatePromptString("text")));
        }
        ...
        private void OnLoadFromDesktop()
        {
            ...
            _dispatcher.BeginInvoke(async () =>
            {
                ...
                foreach (var item in result)
                {
                    ...
                    {
                        // 序列化失败视为 string
                        this.PromptPacketList.Add(CreatePromptPacket(IconList, "text", CreatePromptString($"{temp}")));
                        Debug.WriteLine($"{ex.Message}");
                    }
                }
                ...
                if (files.FirstOrDefault(file => Path.GetFileName(file) == "ContinuePrompt.txt") is string cp)
                {
                    ...
                    this.PromptPacketList2.Add(CreatePromptPacket(IconList, "接续词", CreatePromptString(response)));
                }
    ...
    partial class PromptEditorVM
    {
        ...
        /// 创建 PromptPacket
        /// </summary>
        private PromptPacket CreatePromptPacket(string[] iconList, string text, object content, bool isChecked = false)
        {
            var textIcon = iconList[0];
            var jsonIcon = iconList[1];
            var icon = content switch
            {
                PromptString => textIcon,
                ToggleTreeViewNode => jsonIcon,
                _ => ""
            };
                    ...
                    case PromptString ps:
                        {
                            try
                            {
                                ...
                                promptPacket.IconData = jsonIcon;
                    ...
                    case ToggleTreeViewNode tvn:
                        {
                            try
                            {
                                ...
                                promptPacket.IconData = textIcon;
